...
/**
 * ZFrame The ZFrame class provides methods to send and receive single message frames across 0MQ
 * sockets. A 'frame' corresponds to one underlying zmq_msg_t in the libzmq code. When you read a
 * frame from a socket, the more() method indicates if the frame is part of an unfinished multipart
 * message. The send() method normally destroys the frame, but with the ZFRAME_REUSE flag, you can
 * send the same frame many times. Frames are binary, and this class has no special support for text
 * data. Based on <a href="http://github.com/zeromq/czmq/blob/master/src/zframe.c">zframe.c</a> in
 */
public class ZFrame {
     ...
     *
     * @param data New byte array contents for frame
     */
     ...
     * Sets new contents for frame
     *
     * @param data String contents for frame
     */
     ...
     *
     * @param str String to compare with frame data
     ...
     *
     * @return A text string or hex-encoded string if data contains any non-printable ASCII
     * characters
     */
    ...
    /**
     * Internal method to call recv on the socket. Does not trap any ZMQExceptions but expects
     * caling routine to handle them.
     *
    ...
    private byte[] recv(Socket socket, int flags) {
        try {
            ...
            Error error = ZMQ.Error.findByCode(e.getErrorCode());
    ...
    /**
     * Receives single frame from socket, returns the received frame object, or null if the recv was
     * interrupted. Does a blocking recv, if you want to not block then use recvFrame(socket,
     * ZMQ.DONTWAIT);
     *
    ...
    /**
     * Receive a new frame off the socket, Returns newly-allocated frame, or null if there was no
     * input waiting, or if the read was interrupted.
     *
     * @param socket Socket to read from
     * @param flags Pass flags to 0MQ socket.recv call
     * @return received frame, else null
     */
