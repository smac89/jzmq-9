...
/**
 * Simple certstore that manages certificate file in a directory. Those files need to be in
 * ZMP-Format which is created by ZConf.java
 *
...
//		startDummyCheckThread(1000);
        ...
        for (File f : root.listFiles()) {
            if (f.isFile()) {
                ...
                System.out.println("WARNING:" + f
                                     + " is neither file nor directory? This shouldn't happen....SKIPPING");
            }
        }
    }
    ...
    public boolean containsPublicKey(byte[] publicKey) {
        if (publicKey.length != 32) {
            throw new RuntimeException(
              "publickey needs to have a size of 32bytes. got only " + publicKey.length);
        }
     ...
     *
     * if you have no checkthread running this method will scan the cert-folder for changes on every
     * call
     */
    public boolean containsPublicKey(String publicKey) {
        if (publicKey.length() != 40) {
            throw new RuntimeException(
              "z85 publickeys should have a length of 40bytes but got " + publicKey.length());
        }
        ...
        traverseDirectory(directory, new IFileVisitor() {
            ...
            public void handleFile(File f) {
                try {
                    ...
                    if (publicKey == null) {
                        System.out.println(
                          "Warning!! File has no curve/public-key-element: " + f.getAbsolutePath()
                            + " SKIPPING!");
                        return;
                    }
    ...
    /**
     * Check if files in the cert-folder have been added or removed. Changes are also recognized in
     * subfolder This might differ from OS to OS. At least on Linux this works. Timestamps are
     * second-based!!
     *
     * @return boolean
     */
    ...
    /**
     * Start simple thread that checks if the the directory timestamp changed or the overall count
     * of files did change. If yes it will reload all certificates again
     *
